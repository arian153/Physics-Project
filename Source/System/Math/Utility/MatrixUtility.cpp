...
namespace PhysicsProject
{
    namespace Math
    {
        namespace Matrix44
        {
            ...
            {
                PhysicsProject::Matrix44 result;
            ...
            PhysicsProject::Matrix44 Rotation(const AxisRadian& axis_radian)
            {
                ...
                Real              t    = 1.0f - cos;
                PhysicsProject::Vector3  Axis = axis_radian.axis;
                Axis.SetNormalize();
                Real tx         = t * Axis.x;
                Real ty         = t * Axis.y;
                Real tz         = t * Axis.z;
            ...
            PhysicsProject::Matrix44 Scale(const PhysicsProject::Vector3& scale, Real w)
            {
                PhysicsProject::Matrix44 result;
                result.data[0]  = scale.x;
                result.data[1]  = 0.0f;
            ...
            PhysicsProject::Matrix44 AffineTransformation(const PhysicsProject::Vector3& scale, const PhysicsProject::Vector3& origin, const Quaternion& rotation, const PhysicsProject::Vector3& translation)
            {
                PhysicsProject::Matrix44 result = Scale(scale, 1.0f);
            ...
            PhysicsProject::Matrix44 PerspectiveMatrix(Real screen_aspect, Real field_of_view, Real far_plane, Real near_plane)
            {
            ...
            PhysicsProject::Matrix44 OrthoGraphicMatrix(size_t client_width, size_t client_height, Real far_plane, Real near_plane)
            {
            ...
            PhysicsProject::Matrix44 OrthoGraphicCenterMatrix(Real right, Real left, Real top, Real bottom, Real far_plane, Real near_plane)
            {
                PhysicsProject::Matrix44 result;
            ...
            PhysicsProject::Matrix44 LookAt(const PhysicsProject::Vector3& eye_position, const PhysicsProject::Vector3& focus_position, const PhysicsProject::Vector3& up_direction)
            {
                return LookTo(eye_position, focus_position - eye_position, up_direction);
            }

            PhysicsProject::Matrix44 LookTo(const PhysicsProject::Vector3& eye_position, const PhysicsProject::Vector3& eye_direction, const PhysicsProject::Vector3& up_direction)
            {
                if (eye_direction.IsZero())
                {
                    return PhysicsProject::Matrix44();
                }
                PhysicsProject::Vector3  r2               = eye_direction.Normalize();
                PhysicsProject::Vector3  r0               = CrossProduct(up_direction, r2).Normalize();
                PhysicsProject::Vector3  r1               = CrossProduct(r2, r0);
                PhysicsProject::Vector3  neg_eye_position = eye_position.Negate();
                Real              d0               = DotProduct(r0, neg_eye_position);
                Real              d1               = DotProduct(r1, neg_eye_position);
                Real              d2               = DotProduct(r2, neg_eye_position);
                PhysicsProject::Matrix44 result;
                result.SetColumns(PhysicsProject::Vector4(r0, d0), PhysicsProject::Vector4(r1, d1), PhysicsProject::Vector4(r2, d2), Math::Vector4::W_AXIS);
                return result;
            }
        }
        ...
        namespace Matrix33
        {
            ...
            {
                PhysicsProject::Matrix33 result;
                if (quaternion.IsUnit())
                {
                    Real xs        = quaternion.i + quaternion.i;
                    Real ys        = quaternion.j + quaternion.j;
